"""
Command-line interface for homeassistant-cli.
"""
...
def find_native_binary() -> str:
    ...
    target_binary = project_root / "target" / "release" / "ha"
    if target_binary.exists() and not target_binary.is_dir():
        return str(target_binary)

    if sys.platform == "win32":
        target_binary = project_root / "target" / "release" / "ha.exe"
        if target_binary.exists() and not target_binary.is_dir():
            return str(target_binary)

    raise FileNotFoundError(
        "Could not find the native ha binary. "
        "Please ensure it was built with 'cargo build --release'."
    )


def main() -> int:
    """Run the ha command line tool."""
    try:
